...
module.exports = {
	...
	execute(message) {
		...
		embed.addField('UTC', `${clockEmoji(now.hour, now.minute)} ${now.toISODate()} ${now.toFormat(timeFormat)}`);

		times.forEach(time => {
			const heading = `${time.offsetNameLong} (${time.zoneName.replace('_', ' ')})`;
			const content = `${clockEmoji(time.hour, time.minute)} ${time.toISODate()} ${time.toFormat(timeFormat)}`;
